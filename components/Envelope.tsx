...
export const Envelope: React.FC<EnvelopeProps> = ({ onOpen, isOpened }) => {
  return (
    <div className="relative w-72 h-48 sm:w-96 sm:h-64 cursor-pointer group" onClick={onOpen}>
      <div className="absolute inset-0 bg-[#fdfaf6] rounded shadow-2xl border-2 border-[#d4c5b5] transition-transform duration-1000">
      ...
      >
        {/* Bronze seal */}
        <div className="relative w-16 h-16 sm:w-20 sm:h-20 bg-[#6F3400] rounded-full shadow-xl flex items-center justify-center border-4 border-[#8B4513]">
           <span className="font-serif italic text-white text-xl sm:text-2xl select-none">BM</span>
      ...
      <div className={`absolute bottom-6 left-1/2 -translate-x-1/2 text-center w-full px-4 transition-opacity duration-500 ${isOpened ? 'opacity-0' : 'opacity-100'}`}>
        <p className="font-serif italic text-gray-500 text-lg">You are invited</p>
      </div>

      {!isOpened && (
        <>
          <div className="absolute -top-4 -left-4 w-12 h-12 opacity-30 text-[#8B4513]">
          ...
          <div className="absolute -bottom-4 -right-4 w-12 h-12 opacity-30 text-[#8B4513] rotate-180">
            <svg viewBox="0 0 100 100" fill="currentColor"><path d="M50 0C60 20 80 40 100 50C80 60 60 80 50 100C40 80 20 60 0 50C20 40 40 20 50 0Z" /></svg>
          </div>
        </>
      )}
